...
conn.on('user-presence-update', json => console.log(json.id + ' presence is => ' + json.type)) || console.log('Succes')
conn.on('message-status-update', json =>
{
   const participant = json.participant ? ' (' + json.participant + ')' : '' // participant exists when the message is from a group
   console.log(`[ ${moment().format("HH:mm:ss")} ] => bot`)
})
  ...
  var teks = text.replace(/!nulis /, '')
    ...
    .then((res) => {
      imageToBase64(res.data.result)
        .then(
          (ress) => {
            conn.sendMessage(id, 'Sabar lagi proses oke', MessageType.text)
...
if (text.includes("!randomanime"))
   {
    var items = ["anime girl", "anime cantik", "anime boy", "loli", "waifu cantik", "anime", "beautiful anime", "kawai loli", "kawai anime", "anime loli kawai", "loli kawai", "loli imut", "neko", "waifu"];
...
//new fitur pengganti !animepict beta
if (text.includes("!waifu"))
   {
    var items = ["waifu"];
    var wife = items[Math.floor(Math.random() * items.length)];
    var url = "https://raw.githubusercontent.com/ArugaZ/grabbed-results/main/random/anime/" + wife;
    
    axios.get(url)
      .then((result) => {
        var n = JSON.parse(JSON.stringify(result.data));
        var wif =  n[Math.floor(Math.random() * n.length)];
        imageToBase64(wif) 
        .then(
            (response) => {
	var buf = Buffer.from(response, 'base64'); 
  ...
  if (text.includes("!neko"))
   {
    var items = ["neko"];
    var nek = items[Math.floor(Math.random() * items.length)];
    var url = "https://raw.githubusercontent.com/ArugaZ/grabbed-results/main/random/anime/" + nek;
    
    axios.get(url)
      .then((result) => {
        var n = JSON.parse(JSON.stringify(result.data));
        var ne =  n[Math.floor(Math.random() * n.length)];
        imageToBase64(ne) 
        .then(
            (response) => {
	var buf = Buffer.from(response, 'base64'); 
